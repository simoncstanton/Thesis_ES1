#!/usr/bin/env python3
# file: obs_compile_exp_episode_rewards_tournament.py
'''
    obs_compile_exp_episode_rewards_tournament
    
    usage: python -m obs_compile_exp_episode_rewards_tournament -j __STR__ -z false -l true
        ...
        nominative key
            eo_ep_r   [experiment observation episode reward]
        ...
        - analyses a set of episodes (minimum == 1) across a set of subjobs (minimum == 1)
            ...
            - for each subjob
                - for each trial (matching of two strategies)    
                    - extract final-timestep reward for each agent  (agent_0_terminal_sum, agent_1_terminal_sum)
                        - terminal sum per episode 

                        - write to   ("obs_396172_sj_0_ep_r_terminal_sum_a0")
                            __EXP_ID__/data/ep_r/obs_\__EXP_ID__\_sj_\__SJ__\_ep_r_terminal_sum_a0.csv
                            __EXP_ID__/data/ep_r/obs_\__EXP_ID__\_sj_\__SJ__\_ep_r_terminal_sum_a1.csv
...
def main(argv):
    ...
    '''
    obs_utility().ep_r_assemble_agent_terminal_sum_tournament(result_set_data, obs_data, obs_data_summary)    

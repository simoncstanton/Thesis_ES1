...
import numpy as np
import pandas as pd
# px and/or pd and a dict seem to interact badly, generates "PerformanceWarning: DataFrame is highly fragmented.". see function below 
# write_reward_graph_all_episode_series  [ 18/01/2023 ]
import warnings
...
def write_reward_graph_all_episode_series(obs_data, obs_data_summary, data_a, strategy, sj_id, output_suffix, a_id):
    ...
    for i, e in enumerate(data_a):
        pd_series = pd.Series(data_a[i], index=xaxis_timestep_range)
        data.update({ "episode " + str(i) : pd_series })
        
    # For some reason this next causes Pandas performance warning: "PerformanceWarning: DataFrame is highly fragmented. ...etc".
    # Unclear how to mitiaget it, and is only occuring on this function to write this one set of graphs - that are being done in R for publication anyway
    # so have added the suppress warning after importing pandas, see above. updated plotly, plotly-express and pandas. am still on py3.74  [ 18/01/2023 ]   

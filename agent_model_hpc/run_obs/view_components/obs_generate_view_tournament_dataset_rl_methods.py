...
def main(argv):
    ...
    # max scores 
    reward_upper_bound_game = 13 * 4 * 10000
    mc_upper_bound_game = 13 * 10000
    ...
    for i in range(0, obs_data["obs_exp"]["sj_count"]):
        ...
        j_count = 91    # num valid games in tournament_n^2 
        ...
        for j in data.index.tolist():
            ...
            if j in rlmethods_indices:
                
                # strategy names
                a0.append(rlmethods_a0[k])
                a1.append(rlmethods_a1[k])
                ...
                strategies_game_model[rlmethods_a0[k]][0] += int(data.iloc[j][4])
                strategies_game_model[rlmethods_a1[k]][0] += int(data.iloc[j][5])

                strategies_tournament_total[rlmethods_a0[k]][0] += int(data.iloc[j][4])
                strategies_tournament_total[rlmethods_a1[k]][0] += int(data.iloc[j][5])
                ...
                strategies_game_model[rlmethods_a0[k]][1] += mc
                strategies_game_model[rlmethods_a1[k]][1] += mc
                 
                strategies_tournament_total[rlmethods_a0[k]][1] += mc
                strategies_tournament_total[rlmethods_a1[k]][1] += mc

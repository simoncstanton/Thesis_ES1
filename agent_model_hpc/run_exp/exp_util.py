...
class Exp_utility:
   ...
   # move next set of three functions to common obs exp util [TODO]
    ...
    def set_exp_data_start(self, exp_data):
        ...
        # set to 1 power less, so 1000 timesteps in episode will only write every 100 timesteps.
        # is overriden in tournament exp_type with option to set power value on invocation.
    ...
    def make_exp_job_paths(self, exp_data):
        ...
        if exp_data["exp_invocation"]["localhost"]:
            ...
            self.make_job_path(os.path.join(exp_data["exp_invocation"]["basepath"], os.sep.join(exp_data["job_parameters"]["heartbeat_path"])), exp_data["exp_invocation"]['verbose'])
        ...
        self.make_job_path(os.path.join(exp_data["exp_invocation"]["basepath"], os.sep.join(exp_data["job_parameters"]["journal_path"])), exp_data["exp_invocation"]['verbose'])
    ...
    def make_tournament_trial_folder(self, exp_data, tournament_trial_id):
        ...
        self.make_job_path(tournament_trial_path, exp_data["exp_invocation"]['verbose'])
    ...
    def make_job_episode_paths_tournament(self, exp_data, tournament_trial_path):    
        '''
            ...
            are the same for experiment types: symmetric selfplay, asymmetric_selfpay, selfplay_parameter_study, round_robin and tournament
        ...
        self.make_job_path(cumulative_outcome_history_path, exp_data["exp_invocation"]['verbose'])
        
        map_outcome_history_path = os.path.join(tournament_trial_path, self.hpc_config["exp_data_leaf_dirs"]["map_outcome_history"])
    ...
    '''
        Tournament write functions
            
            - we don't write agent action history for tournaments, only 
    ...
    # write_agent_reward_history_sum_episodes_tournament
    
    def write_agent_reward_history_sum_episodes_tournament(self, exp_data, agent_id, agent_reward_episode_totals, tournament_trial_id):
      
        history_path = os.path.join(exp_data["job_parameters"]['job_dir'], str(tournament_trial_id), self.hpc_config["exp_data_leaf_dirs"]["reward_history"])
        file = exp_data["job_parameters"]['data_filename_prefix'] + "_"+ str(tournament_trial_id) + "_" + "agent_" + str(agent_id) + "_reward_history" + "_" + "sum_episodes"
    ...
    def write_single_episode_outcomes_timestep_map_tournament(self, exp_data, episode, single_episode_outcomes_timestep_map, tournament_trial_id):
        o_d_path = os.path.join(exp_data["job_parameters"]['job_dir'], str(tournament_trial_id), self.hpc_config["exp_data_leaf_dirs"]["map_outcome_history"])
        
        for o, d in single_episode_outcomes_timestep_map.items():
            file = exp_data["job_parameters"]['data_filename_prefix'] + "_" + str(tournament_trial_id) + "_outcomes_" + "episode_" + str(episode+1) + "_" + o + "_map"
            
            if exp_data["exp_invocation"]["compress_writes"]:
                file = file + ".csv.gz"
                self.write_compressed_csv(single_episode_outcomes_timestep_map[o], os.path.join(o_d_path, file))
            else:
                file = file + ".csv"
                self.write_csv(single_episode_outcomes_timestep_map[o], os.path.join(o_d_path, file))
                
                
    def write_sum_episode_outcomes_cumulative_tournament(self, exp_data, outcome, sum_episode_outcome_cumulative, tournament_trial_id):
        o_d_path = os.path.join(exp_data["job_parameters"]['job_dir'], str(tournament_trial_id), self.hpc_config["exp_data_leaf_dirs"]["cumulative_outcome_history"])
        file = exp_data["job_parameters"]['data_filename_prefix'] + "_" + str(tournament_trial_id) + "_" + outcome + "_outcomes_" + "sum_episodes_cumulative"
    ...
    '''
        heartbeat tournament
            tournament duplicates heartbeat functions to add tournament_trial_id
            power value (set elsewhere) can be overriden on invocation for tournament exp_type
    ...
    def append_episode_cumulative_outcome_sum(self, sum_episode_outcomes_cumulative, episode, trial):
        sum_episode_outcomes_cumulative["cc"][episode] = trial.get_CC()
        sum_episode_outcomes_cumulative["cd"][episode] = trial.get_CD()
        sum_episode_outcomes_cumulative["dc"][episode] = trial.get_DC()
        sum_episode_outcomes_cumulative["dd"][episode] = trial.get_DD()
